...
class ControllerCatalogProduct extends \Ninja\AdminController
{
    private $error = array();
        ...
        */
        ...
        foreach ($results as $result) {
            ...
            $data['products'][] = array(
                ...
                'special' => $special,
                'weight' => $result['weight'] > 0 ? $this->getWeight()->format($result['weight'], $result['weight_class_id']) : '',
//				'length'     => $this->getLength()->format()
        ...
        //Excel Import
        $data['excel_import'] = $this->getUrl()->link('catalog/excel_import', 'user_token=' . $this->getSession()->data['user_token'], true);
    ...
    {
        $this->getDocument()->addScript('view\javascript\jsbarcode\JsBarcode.all.min.js');
    ...
    public function updateStatus()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $product_id = $this->request->isset_post('product_id');
            $status = $this->request->isset_post('status');

            if (!empty($product_id)) {
                $this->load->model('catalog/product');
                if ($this->model_catalog_product->updateStatus($product_id, $status)) {
                    $isError = false;
                }
            }
        }
