...
class ControllerMarketingCoupon extends AdminController
{
    ...
    protected function getList()
    {
        ...
        $data['settings'] = $this->getUrl()->link($this->route.'/settings', 'user_token=' . $this->getUserToken());
    ...
    public function settings()
    {
        ...
        if ($this->getRequest()->isRequestMethodPost()) {
            ...
            $this->model_setting_setting->editSetting('coupon_settings', $this->getRequest()->post);

            $this->getSession()->setSuccessMessage($this->getLanguage()->get('text_success'));
        }
        ...
        $data['text_form'] = $this->getLanguage()->get('text_settings');
        $data['coupons'] = $this->model_marketing_coupon->getCoupons(['limit' => 1000]);

        $options_coupons = [];
        foreach ($data['coupons'] as $coupon) {
            $options_coupons[$coupon['coupon_id']] = $coupon['name'];
        }

        $form = new Form($this->registry, $this->getUrl()->link($this->route.'/settings', 'user_token=' . $this->getUserToken()));
        $form->addSelect('coupon_settings_coupon_on_registration_id', $options_coupons);
        $form->setValues([
            'coupon_settings_coupon_on_registration_id' => $this->getFromPostOrConfig('coupon_settings_coupon_on_registration_id')
        ]);
        $data['form'] = $form->renderWholeForm();

        $data['cancel'] = $this->getUrl()->link($this->route, 'user_token=' . $this->getUserToken());
        ...
        $this->getResponse()->setOutput($this->getLoader()->view($this->route . '_settings', $data));
    }
