...
class ControllerSettingSetting extends \Ninja\AdminController
{
	private $error = array();
...
//		}
		...
		$formRender = new Tools\Monsters\Language\Twig\FormRenderer\FormRendererBootstrap($this->registry);
		$tabGeneral = $formRender->tabs->addTab('tab-general', '');

		$tabGeneral->addControl('input', 'text','config_meta_title',
            $this->getLanguage()->get('entry_meta_title'),
            'entry_meta_title',
            !empty($this->error['meta_title']),
            (!empty($this->error['meta_title']) ? $this->error['meta_title'] : '')
        );

        $tabGeneral->addControl('textarea', '','config_meta_description',
            $this->getLanguage()->get('entry_meta_description'), 'input-meta-description');

        $tabGeneral->addControl('textarea', '','config_meta_keyword',
            $this->getLanguage()->get('entry_meta_keyword'), 'input-meta-keyword');

        $data['general_tabs'] = $formRender->generateLanguagesTabs('tab-general');
        $data['general_form'] = $formRender->generateFormElements('tab-general');


        $tabStore = $formRender->tabs->addTab('tab-store', '');

        $tabStore->addControl('input', 'text', 'config_name',
            $this->getLanguage()->get('entry_name'),
            'input-name',
            !empty($this->error['name']),
            (!empty($this->error['name']) ? $this->error['name'] : '')
        );

        $tabStore->addControl('input', 'text', 'config_owner',
            $this->getLanguage()->get('entry_owner'),
            'input-owner',
            !empty($this->error['owner']),
            (!empty($this->error['owner']) ? $this->error['owner'] : '')
        );

        $tabStore->addControl('textarea', '','config_address',
            $this->getLanguage()->get('entry_address'), 'input-address',
            !empty($this->error['address']),
            (!empty($this->error['address']) ? $this->error['address'] : ''));

        $tabStore->addControl('textarea', '','config_open',
            $this->getLanguage()->get('entry_open'), 'input-open');

        $tabStore->addControl('textarea', '','config_comment',
            $this->getLanguage()->get('entry_comment'), 'input-comment');


        $data['store_tabs'] = $formRender->generateLanguagesTabs('tab-store');
        $data['store_form'] = $formRender->generateFormElements('tab-store');
