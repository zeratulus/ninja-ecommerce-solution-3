...
class ControllerBlogReview extends \Ninja\NinjaController {
	private $error = array();
	...
	protected function getList() {
		...
		foreach ($results as $result) {
            if (!empty($result['avatar'])) {
                $image = $this->model_tool_image->resize("avatars/article/{$result['avatar']}", 60, 60);
            } else {
	...
	protected function getForm() {
        ...
        if (!empty($review_info['avatar'])) {
            $data['avatar'] = $this->model_tool_image->resize("avatars/article/{$review_info['avatar']}", 300, 300);
        } else {
