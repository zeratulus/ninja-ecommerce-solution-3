...
class ModelBlogArticle extends Model {
	...
	public function getArticle($article_id) {
		...
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, pd.description AS description, bcd.name AS category_title, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review_article r1 WHERE r1.article_id = p.article_id AND r1.status = '1' GROUP BY r1.article_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review_article r2 WHERE r2.article_id = p.article_id AND r2.status = '1' GROUP BY r2.article_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "article p LEFT JOIN " . DB_PREFIX . "article_description pd ON (p.article_id = pd.article_id) LEFT JOIN ".DB_PREFIX."article_to_blog_category a2bc ON (a2bc.article_id = p.article_id) LEFT JOIN ".DB_PREFIX."blog_category_description bcd ON (a2bc.blog_category_id = bcd.blog_category_id) LEFT JOIN " . DB_PREFIX . "article_to_store p2s ON (p.article_id = p2s.article_id) WHERE p.article_id = '" . (int)$article_id . "' AND bcd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
		
		if ($query->num_rows) {
			return array(
                'category_title'   => $query->row['category_title'],
	...
	public function getArticles($data = array()) {
		...
		if (!$article_data) {
			$sql = "SELECT *, p.article_id, bcd.name AS category_title, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review_article r1 WHERE r1.article_id = p.article_id AND r1.status = '1' GROUP BY r1.article_id) AS rating FROM " . DB_PREFIX . "article p LEFT JOIN " . DB_PREFIX . "article_description pd ON (p.article_id = pd.article_id) LEFT JOIN " . DB_PREFIX . "article_to_store p2s ON (p.article_id = p2s.article_id) LEFT JOIN ".DB_PREFIX."article_to_blog_category AS a2bc ON (a2bc.article_id = p.article_id) LEFT JOIN ".DB_PREFIX."blog_category_description AS bcd ON (a2bc.blog_category_id = bcd.blog_category_id) ";
						
			if (!empty($data['filter_blog_category_id'])) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "article_to_blog_category a2c ON (p.article_id = a2c.article_id)";			
			}
			
			$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND bcd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";
	...
	public function getArticlesList($language_id, $blog_category_id = null) {
		$sql = "SELECT *, pd.name as name, p.article_id, bcd.name AS category_title, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review_article r1 WHERE r1.article_id = p.article_id AND r1.status = '1' GROUP BY r1.article_id) AS rating FROM " . DB_PREFIX . "article p LEFT JOIN " . DB_PREFIX . "article_description pd ON (p.article_id = pd.article_id) LEFT JOIN " . DB_PREFIX . "article_to_store p2s ON (p.article_id = p2s.article_id) LEFT JOIN ".DB_PREFIX."article_to_blog_category AS a2bc ON (a2bc.article_id = p.article_id) LEFT JOIN ".DB_PREFIX."blog_category_description AS bcd ON (a2bc.blog_category_id = bcd.blog_category_id) ";
		$sql .= " WHERE pd.language_id = '" . (int)$language_id . "' AND bcd.language_id = '" . (int)$language_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";

		if (!is_null($blog_category_id)) {
			$sql .= " AND a2bc.blog_category_id = '" . (int)$blog_category_id . "'";
		}
