...
class ControllerBlogArticle extends \Ninja\NinjaController {
	private $error = array();
	...
	public function index() {
		...
		$configblog_name = $this->config->get('configblog_name' . $this->language_id);
		...
		if ($article_info) {
			...
			$this->document->addScript('catalog/view/javascript/jquery/magnific/jquery.magnific-popup.min.js', 'header', 'defer');
            $this->document->addScript('catalog/view/theme/default/assets/barrating/jquery.barrating.min.js', 'header', 'defer');
            $this->document->addScript('catalog/view/theme/default/assets/croppie/croppie.min.js', 'header', 'defer');
            $this->document->addStyle('catalog/view/javascript/jquery/magnific/magnific-popup.css');
            $this->document->addStyle('catalog/view/theme/default/assets/croppie/croppie.css');
            $this->document->addStyle('catalog/view/theme/default/assets/barrating/themes/fontawesome-stars-o.css');
            ...
            if ($article_info['image']) {
                ...
                $data['popup'] = 'image/' . $article_info['image'];
            } else {
                $data['image'] = '';
                $data['popup'] = '';
            }

            if ($article_info['date_added']) {
			...
			//OpenGraph Image routine
			$og_image = $article_info['og_image'];
			//change image on primary image
			if (empty($og_image)) {
				$og_image = $article_info['image'];
			}
			//change image on logo
			if (empty($og_image)) {
				$og_image = $this->getConfig()->get('config_logo');
			}
			$og_image_cache = $this->model_tool_image->resize($og_image, 1024, 1024);
			$this->getDocument()->setOgImage($og_image_cache);
			//End of OpenGraph Image routine
	...
	public function review() {
		...
		$results = $this->model_blog_review->getReviewsByArticleId($this->request->get['article_id'], ($page - 1) * $review_per_page, $review_per_page);

		foreach ($results as $result) {
            if (!empty($result['avatar'])) {
                $image = $this->model_tool_image->resize("avatars/article/{$result['avatar']}", 300, 300);
            } else {
