...
class ControllerMailOrder extends \Ninja\NinjaController
{
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		// HTML Mail
        $mailUtils = new Mail\MailUtils($this->registry);
...
//		$data['logo'] = $order_info['store_url'] . 'image/' . $this->config->get('config_logo');
		...
		$this->getLoader()->model('tool/upload');
		...
		if (!$from) {
			$from = $this->getConfig()->get('config_email');
		}
        $data['email_from'] = $from;
        $data['email_to'] = $order_info['email'];
		$data['sender'] = html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8');
		$data['subject'] = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
		$data['html'] = $this->getLoader()->view('mail/order_add', $data);

		$mailUtils->trySendMail('order', $data);
	}
	...
	public function edit($order_info, $order_status_id, $comment) {
        ...
        $data['sender'] = html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8');
        $data['subject'] = html_entity_decode(sprintf($language->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
        $data['html'] = $this->load->view('mail/order_edit', $data);

        $mailUtils->trySendMail('order-edit', $data);
	}
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
            ...
            $data['email_to'] = $this->getConfig()->get('config_email');
            $data['sender'] = html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8');
            $data['subject'] = html_entity_decode(sprintf($this->getLanguage()->get('text_subject'), $order_info['store_name'], $order_info['order_id']), ENT_QUOTES, 'UTF-8');
            $data['html'] = $this->getLoader()->view('mail/order_add', $data);
            $mailUtils->trySendMail('mail/order_alert', $data);
			...
			foreach ($emails as $email) {
				if ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    $re = [];
                    $re['unsubscribe'] = $data['unsubscribe'];
                    $re['logo'] = $data['logo'];
                    $re['email_from'] = $data['email_from'];
                    $re['email_to'] = $email;
                    $re['sender'] = $data['sender'];
                    $re['subject'] = $data['subject'];
                    $re['html'] = $this->getLoader()->view('mail/order_add', $re);
                    $mailUtils->trySendMail('mail/order_alert', $re);
				}
			}
